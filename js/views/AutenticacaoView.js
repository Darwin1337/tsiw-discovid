...
export default class AutenticacaoView {
  constructor() {
    ...
    // Bind do select que troca o tipo de registo (normal ou entidade)
  ...
  LoadNormalRegisterInputs() {
    ...
    this.NormalUser_RegisterSubmit();
  }
      ...
      if (document.querySelector("#TipoUser").value == "Utilizador normal") {
        ...
        document.getElementsByClassName("change-type")[0].innerHTML = `
          <form id="form-reg-normal">
            ...
            <div class="mb-2">
              <input id="RegistoPASSWORD" type="password" class="input-login-registar" name="password" placeholder="Password" value="" required>
            </div>
          ...
          <div>
            ...
            <div>
              <button type="submit" class="btn btn-azul-pri" data-bs-toggle="modal" data-bs-target="#add-morada">Adicionar morada</button>
            </div>
            <div class="w-100 my-4">
              <button type="button" class="btn btn-laranja w-100" id="href-submit">Efetuar registo</button>
            </div>
          </div>`;
        ...
        this.NormalUser_AddAddress();

        // Event listener para quando o botão de guardar edição de morada é pressionado
        this.NormalUser_SaveAddressEdition();

        // O botão presente no formulário está escondido (id = RegistoSUBMIT) por isso o botão que está vísivel (id = href-submit), quando carregado irá simular o click no botão escondido.
        ...
        document.getElementById("href-submit").addEventListener("click", event => {
          document.getElementById("RegistoSUBMIT").click();
        });
      } else {
        // Injeta o formulário de registo do utilizador entidade
        document.getElementsByClassName("change-type")[0].innerHTML = `
          <div class="mb-2">
        ...
        // Ativa o select dos tipos de teste
        $('.js-example-basic-multiple').select2();
        //$(".change-type").attr("id", "form-reg-entidade");

        // Dar bind aos inputs do registo do utilizador entidade
        this.LoadPostoRegisterInputs();
      }
    });
  }
    ...
    document.querySelector("#btn-add-morada").addEventListener("click", event => {
      ...
      if (usableAddresses < 5) {
        ...
        if (etiqueta.value && morada.value && codpostal.value && localidade_id != $('.select-localidades').find('option').first().val()) {
          ...
          this.NormalUser_EditAddress();
          this.NormalUser_RemoveAddress();
  ...
  NormalUser_SaveAddressEdition() {
    document.querySelector("#btn-edit-morada").addEventListener("click", event => {
      ...
      const arrIdx = parseInt(event.target.classList[event.target.classList.length - 1]);
      ...
      if (etiqueta.value && morada.value && codpostal.value && localidade_id.find(':selected').val() != localidade_id.find('option').first().val()) {
        ...
        if (this.normalUserAddresses[arrIdx][5]) {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            if (this.normalUserAddresses[id - 1][5]) {
              ...
              if (wasAnotherPossibleAddressFound) {
