...
export default class MarcacoesController {
    ...
    AddEstado(nome) {
      const newId = this.estados.length > 0 ? this.estados[this.estados.length - 1].id + 1 : 1;
      this.estados.push(new DetalhesEstadoModel(newId, nome));
      localStorage.setItem("estados", JSON.stringify(this.estados));
    }

    AddResultado(nome) {
      const newId = this.resultados.length > 0 ? this.resultados[this.resultados.length - 1].id + 1 : 1;
    ...
    AddNewMarcacao(id_utilizador, id_entidade, data_marcacao, id_teste, call_me){
      const newId = this.marcacoes.length > 0 ? this.marcacoes[this.marcacoes.length - 1].id + 1 : 1;
